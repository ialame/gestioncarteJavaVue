...
@Component
@CacheConfig(cacheNames = "WikiDexNetParser")
public class WikiDexNetParser extends AbstractWikiParser {

	private static final Logger LOGGER = LogManager.getLogger(WikiDexNetParser.class);

	@Autowired
	private IHTMLParser htmlParser;


	@Value("${wikidex-net.url}")
	private String wikiDexNetUrl;

	public WikiDexNetParser() {
		super("wiki-es-wikidex", Localization.SPAIN);
	}
		...
		return wikiUrls.stream()
				.<Element>mapMulti((wikiUrl, downstream) -> {
					try {
						...
						elements.select("table.wiki.sortable").select("tbody").select("tr").forEach(downstream);
					} catch (IOException e) {
	...
	public String getUrl(PokemonSetDTO set, @Nonnull List<WikiUrlDTO> wikiUrls) {
		for (var wikiUrl : wikiUrls) {
			...
			var translation = set.getTranslations().get(Localization.SPAIN);
	...
	private Element get(String url) throws IOException {
		return htmlParser.get(htmlParser.processUrl(this.wikiDexNetUrl, URLDecoder.decode(url, StandardCharsets.UTF_8))).body();
	}
}
